...
//        dd($this->clientSecret);
        ...
        // Resolve credentials: raw JSON, deflated string, or file path (legacy)
        $secret = $this->clientSecret;
        if (!json_validate($secret)) {
            // Try inflating as deflated base64 — used when JSON_AUTH_DEFLATED is the source
            try {
                $inflated = $this->inflate($secret);
                if (json_validate($inflated)) {
                    $secret = $inflated;
                }
            } catch (\Throwable) {
                // Not deflated — fall through and pass as-is (file path)
            }
        }
        $config = json_validate($secret) ? json_decode($secret, true) : $secret;
